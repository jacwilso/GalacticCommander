...
public class FiringZone : MonoBehaviour {
    ...
    private void OnEnable()
    {
        ...
        frustrums = new Plane[6,5];
        Vector3 pos = transform.position;
        for (int i = 0; i < 6; i++)
        {
            int indx = i * 11;
            frustrums[i,0] = new Plane(
                pos + 0.5f * Vector3.Scale(innerScale, cubeVerticies[winding[indx]]), 
                pos + 0.5f * Vector3.Scale(innerScale, cubeVerticies[winding[++indx]]), 
                pos + 0.5f * Vector3.Scale(innerScale, cubeVerticies[winding[++indx]])
            );
            for (int j = 0; j < 4; j++)
                frustrums[i,j + 1] = new Plane(
                    pos + 0.5f * Vector3.Scale(outerScale, cubeVerticies[winding[++indx]]),
                    pos + 0.5f * Vector3.Scale(innerScale, cubeVerticies[winding[indx]]),
                    pos + 0.5f * Vector3.Scale(innerScale, cubeVerticies[winding[++indx]])
                );
        }
    ...
    public Face FrustrumFace(Vector3 point)
    {
        for (int i = 0; i < 6; i++)
        {
            ...
            for (int j = 0; j < 5; j++)
            {
                ...
                if (!frustrums[i,j].GetSide(point))
                {
                    inFront = false;
                    break;
                }
